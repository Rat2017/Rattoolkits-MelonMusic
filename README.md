...
├── overlay.js           悬浮窗页面逻辑，展示歌曲信息、封面和同步歌词
├── overlay.css          悬浮窗样式
├── panel.html           控制面板页面（HTML）
├── panel.js             控制面板逻辑，位置调节、尺寸/缩放控制、快捷键绑定、登录管理
├── panel.css            控制面板样式
...
在控制面板的"操作"标签页中，可以分别为键盘、鼠标、手柄绑定操作：
...
在"控件"标签页中，提供完整的悬浮窗调节功能：

**位置** 通过 X/Y 滑块精确定位，或在虚拟桌面上直接拖拽悬浮窗图标
**预设** 点击九宫格预设按钮快速定位到屏幕的指定位置
**尺寸** 分别调整 X/Y 轴尺寸控制背景框大小，字体大小独立控制内容缩放比例
**歌词补偿** 以 0.1s 步进调整歌词同步偏移（±3s 范围）
**透明度** 调节悬浮窗透明度
**行为开关** 控制按钮/歌词/上下曲信息显示、关闭行为等
